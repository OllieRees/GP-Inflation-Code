...
obj.fun = makeSingleObjectiveFunction(
    ...
    fn = function(ps) {
      ...
      wsMean <- ps[2]
      wsVar <- ps[3]
      iVars <- ps[4:length(ps)]
      cov.mat <- cov.mat.func(x, p, lp, leq, kVar, oDoF, iVars, wsMean, wsVar)
      marginal.likelihood.log(y, cov.mat)
    },
    
    par.set = makeParamSet(
      makeNumericParam(id = "oDoF", lower = 0, upper = 3), 
      
      makeNumericParam(id = "wsMean", lower = 0, upper = 10),
      makeNumericParam(id = "wsVar", lower = 0, upper = 10),
  ...
  des <- generateDesign(n = (3 + ncol(x)^2) * 3, getParamSet(obj.fun), fun = lhs::geneticLHS)
  des$y <- apply(des, 1, function(xs) obj.fun(list(xs[1], xs[2], xs[3], xs[4:length(xs)])))
