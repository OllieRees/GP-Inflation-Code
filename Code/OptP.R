...
periodic.kernel.point <- function(x1, x2, p, l, var) {
  euclidean <- sum( (x1 - x2)^2 )
  periodic.sin.expr <- sin(pi/p * sqrt(euclidean))
  periodic.kernel <- exp(- 2/l^2 * periodic.sin.expr^2)
  var * periodic.kernel
}

periodic.kernel <- function(X, p, l, var) {
  x.size <- nrow(X)
  K <- matrix(0, nrow = x.size, ncol = x.size)
  for(i in 1:x.size) {
    for(j in 1:x.size) {
      K[i, j] <- periodic.kernel.point(X[i, ], X[j, ], p, l, var)
    }
  }
  K
}

periodic.partial.diff.point <- function(x, i, j, d, p, l, var) {
  x1 <- x[i]
  x2 <- x[j]
  x1d <- x[i, d]
  x2d <- x[j, d]
  p.sin <- sin(pi/p * sqrt(sum( (x1 - x2)^2 )))
  p.k <- exp(-2/l^2 * p.sin^2)
  g1 <- -4/l^2 * sin(pi/p * sqrt(sum( (x1 - x2)^2 ))) * cos(pi/p * sqrt(sum( (x1 - x2)^2 ))) * pi/p * (x1d - x2d)/sqrt(sum( (x1 - x2)^2 ))
  var * p.k * g1 
}

periodic.partial.diff <- function(x, p, l, var, wMu, wVar) {
  N <- nrow(x)
  D <- ncol(x)
  IK <- c()
  ...
  for (i in 1:N) {
    ...
    for (d in 1:D) {
      samedim <- c()
      for (j in 1:N) {
        samedim[j] <- periodic.partial.diff.point(x, i, j, d, p, l, var)
...
cov.mat.func <- function(x, p, l, kVar, oDof, iVars, wsMean, wsVar) {
  outputNoiseVar <- diag(rchisq(nrow(x), oDof))
  K <- periodic.kernel(x, p, l, kVar)
  d <- periodic.partial.diff(x, p, l, kVar, wsMean, wsVar)
...
obj.fun = makeSingleObjectiveFunction(
  ...
  fn = function(ps) {
    ...
    wsMean <- ps[5]
    wsVar <- ps[6]
    iVars <- ps[7:length(ps)]
    cov.mat <- cov.mat.func(x, p, l, kVar, oDoF, iVars, wsMean, wsVar)
    marginal.likelihood.log(y, cov.mat)
  },
  
  par.set = makeParamSet(
    makeNumericParam(id = "p", lower = 0, upper = 22),
    makeNumericParam(id = "l", lower = 0, upper = 2),
...
des <- generateDesign(n = (6 + ncol(x)^2) * 3, getParamSet(obj.fun), fun = lhs::geneticLHS)
des$y <- apply(des, 1, function(xs) obj.fun(list(xs[1], xs[2], xs[3], xs[4], xs[5], xs[6], xs[7:length(xs)])))
