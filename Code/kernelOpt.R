...
cov.mat.func <- function(x, p, lp, leq, kVar) {
...
obj.fun = makeSingleObjectiveFunction(
    ...
    fn = function(ps) {
      ...
      leq <- ps[3]
      kVar <- ps[4]
      cov.mat <- cov.mat.func(x, p, lp, leq, kVar)
      marginal.likelihood.log(y, cov.mat)
    },
    
    par.set = makeParamSet(
      makeNumericParam(id = "p", lower = 0, upper = 22),
      makeNumericParam(id = "lp", lower = 0, upper = 10),
  ...
  des <- generateDesign(n = 12, getParamSet(obj.fun), fun = lhs::geneticLHS)
  des$y <- apply(des, 1, function(xs) obj.fun(list(xs[1], xs[2], xs[3], xs[4])))
